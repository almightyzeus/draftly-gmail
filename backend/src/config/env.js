...
function parsePort(portStr) {
    ...
    if (!Number.isInteger(port) || port < 1 || port > 65535) {
        throw new Error(`Invalid PORT: must be an integer between 1 and 65535, got: ${portStr}`);
    }
    return port;
}
/**
...
function parseEncryptionKey(base64Key) {
    try {
        ...
        if (buffer.length !== 32) {
            throw new Error(`Encryption key must be 32 bytes (256 bits) when decoded, got: ${buffer.length} bytes`);
        }
        return buffer;
    }
    catch (error) {
        ...
        throw new Error(`DATA_ENCRYPTION_KEY_BASE64 must be valid base64. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 * Validate URL format
...
function loadEnv() {
    return {
        nodeEnv: (['development', 'production', 'test'].includes(process.env.NODE_ENV || '')
        ...
        encryptionKey: parseEncryptionKey(requireEnv('DATA_ENCRYPTION_KEY_BASE64', 'Base64-encoded 32-byte encryption key')),
        openai: {
        ...
        google: {
            ...
            redirectUri: validateUrl(requireEnv('GOOGLE_REDIRECT_URI', 'Google OAuth2 redirect URI'), 'GOOGLE_REDIRECT_URI'),
        },
        frontendUrl: validateUrl(process.env.FRONTEND_URL ?? 'http://localhost:4200', 'FRONTEND_URL'),
    };
}
